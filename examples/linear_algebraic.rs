...
fn test_full_linear_search() -> (f64, usize) {
    ...
    for mask_idx in 0..total_masks {
        ...
        for _ in 0..n {
            let mut state = sample_rng.next_state();
            let in_par: u64 = (0..STATE_WORDS).map(|w| parity(state[w] & alpha[w])).fold(0, |a, b| a ^ b);
            kk_permute_local(&mut state, 32);
            let out_par: u64 = (0..STATE_WORDS).map(|w| parity(state[w] & beta[w])).fold(0, |a, b| a ^ b);
            if in_par == out_par {
                agree_count += 1;
            }
        }
...
fn mfr_derivative_nonzero(
    a0: u64, b0: u64, rot: u32,
    da: &[u64], db: &[u64], k: usize,
) -> bool {
    let mut result = 0u64;
...
fn quintet_derivative_nonzero(
    base: [u64; 5], rot: [u32; 2],
    directions: &[[u64; 5]], k: usize,
) -> bool {
...
fn test_quintet_degree() -> usize {
    ...
    for order in 2..=max_order {
        ...
        for _ in 0..num_trials {
            ...
            let dirs: Vec<[u64; 5]> = (0..order).map(|_| {
...
fn permutation_derivative_nonzero(
    base: &KkState, rounds: usize,
    directions: &[KkState], k: usize,
) -> bool {
...
fn main() {
    ...
    println!("  At: input bit {} ({}), output bit {}",
        mfr_ipos,
    ...
    if !mfr_lin_pass { all_pass = false; }
    ...
    println!("  At: input bit {} ({}), output bit {}",
        ddr_ipos,
        if ddr_ipos < 64 { "data word a" } else { "control word b" },
        ddr_opos);
    ...
    println!("  {} samples per mask, {} random masks per round count", 1u64 << 16, 200);
    let multiround_lin = test_multiround_linear();
    let noise_floor_16 = (1.0 / (1u64 << 16) as f64).sqrt(); // 1/√N
    println!("  {:>6} {:>12} {:>12}", "Rounds", "Max |bias|", "log₂|bias|");
    ...
    let four_round_bias = multiround_lin.iter()
        .find(|&&(r, _, _)| r == 4)
        .map(|&(_, b, _)| b)
        .unwrap_or(1.0);
    ...
    println!("  {} samples per mask, sparse + medium + dense masks...", 1u64 << 17);
    let (full_lin_max, full_lin_masks) = test_full_linear_search();
    let full_log2 = if full_lin_max > 0.0 { full_lin_max.log2() } else { -64.0 };
    ...
    println!("  Verdict: {} (max 32-round |bias| < 0.01)\n",
        if full_lin_pass { "PASS ✅" } else { "FAIL ❌" });
    if !full_lin_pass { all_pass = false; }
    ...
    println!("  Measured algebraic degree: ≥{}{}", mfr_deg,
    ...
    println!("  Measured algebraic degree: ≥{}{}", quintet_deg,
        if quintet_at_limit { " (exceeded test limit)" } else { "" });
    ...
    for &(rounds, deg) in &degree_growth {
        let note = if deg >= 22 { " (exceeded test limit)" } else { "" };
    ...
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    println!("  LINEAR:");
    println!("    MFR max |bias|:         {:.6} (2^{:.1})", mfr_max, mfr_log2);
    println!("    DDR max |bias|:         {:.6} (2^{:.1})", ddr_max, ddr_log2);
    println!("    4-round max |bias|:     {:.6}", four_round_bias);
    println!("    32-round max |bias|:    {:.6} (2^{:.1})", full_lin_max, full_log2);
    println!();
