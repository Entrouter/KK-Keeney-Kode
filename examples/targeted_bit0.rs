...
fn main() {
    ...
    fn ddr(a: u64, b: u64) -> u64 {
        ...
        let s = (f ^ (f >> 16) ^ (f >> 8)) & 63;
        let mut v = a;
        let m = 0u64.wrapping_sub(s & 1); v = (v & !m) | (v.rotate_left(1) & m);
        let m = 0u64.wrapping_sub((s >> 1) & 1); v = (v & !m) | (v.rotate_left(2) & m);
        let m = 0u64.wrapping_sub((s >> 2) & 1); v = (v & !m) | (v.rotate_left(4) & m);
        let m = 0u64.wrapping_sub((s >> 3) & 1); v = (v & !m) | (v.rotate_left(8) & m);
        let m = 0u64.wrapping_sub((s >> 4) & 1); v = (v & !m) | (v.rotate_left(16) & m);
        let m = 0u64.wrapping_sub((s >> 5) & 1); v = (v & !m) | (v.rotate_left(32) & m);
        v
    }
    #[inline(always)]
    fn qr(a: &mut u64, b: &mut u64, c: &mut u64, d: &mut u64, e: &mut u64, rot: [u32; 2]) {
        *a = mfr(*a, *b, rot[0]); *c ^= *a; *d = ddr(*d, *c); *e = mfr(*e, *d, rot[1]); *b ^= *e;
    }
    fn perm(state: &mut [u64; 25], rots: &[[u32; 2]; 15], rounds: usize) {
        for round in 0..rounds as u64 {
            for (row, rot) in rots.iter().enumerate().take(5) {
                let b = row * 5;
                let (mut s0,mut s1,mut s2,mut s3,mut s4) = (state[b],state[b+1],state[b+2],state[b+3],state[b+4]);
                qr(&mut s0,&mut s1,&mut s2,&mut s3,&mut s4,*rot);
                state[b]=s0;state[b+1]=s1;state[b+2]=s2;state[b+3]=s3;state[b+4]=s4;
            }
            for col in 0..5usize {
                let (mut s0,mut s1,mut s2,mut s3,mut s4) = (state[col],state[col+5],state[col+10],state[col+15],state[col+20]);
                qr(&mut s0,&mut s1,&mut s2,&mut s3,&mut s4,rots[5+col]);
                state[col]=s0;state[col+5]=s1;state[col+10]=s2;state[col+15]=s3;state[col+20]=s4;
            }
            for d in 0..5usize {
                let [i0,i1,i2,i3,i4] = DIAGS[d];
                let (mut s0,mut s1,mut s2,mut s3,mut s4) = (state[i0],state[i1],state[i2],state[i3],state[i4]);
                qr(&mut s0,&mut s1,&mut s2,&mut s3,&mut s4,rots[10+d]);
                state[i0]=s0;state[i1]=s1;state[i2]=s2;state[i3]=s3;state[i4]=s4;
            }
            ...
            if round % 8 == 7 {
                for i in 0..19 { state[i] ^= state[19+(i%6)].rotate_left(round as u32); }
            }
        }
    }
    struct Rng(u64);
    impl Rng {
        ...
        fn state(&mut self)->[u64;25]{let mut s=[0u64;25];for w in s.iter_mut(){*w=self.next();}s}
    }
    ...
    for &tw in &a_words {
        ...
        println!("  word {tw:2} bit 0: zero_cap={zero_cap}/{trials} ({r:.4e})  min_total_h={min_h}");
    }
    ...
    for &tw in &b_words {
        ...
        let r = zero_cap as f64 / trials as f64;
        println!("  word {tw:2} bit 1: zero_cap={zero_cap}/{trials} ({r:.4e})  min_total_h={min_h}");
    }
}
